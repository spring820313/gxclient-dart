library testbitcoin.src.crypto;
...
class DoubleSHA256Digest extends BaseDigest {
  ...
  @override
  void update(Uint8List inp, int inpOff, int len) => _internal.update(inp, inpOff, len);
 ...
 */
Uint8List singleDigest(Uint8List input) => new SHA256Digest().process(input);

/**
 * Calculates the double-round SHA-256 hash of the input data.
 */
Uint8List doubleDigest(Uint8List input) => new DoubleSHA256Digest().process(input);
 ...
 */
Uint8List ripemd160Digest(Uint8List input) =>
    new RIPEMD160Digest().process(input);

/**
 * Calculates the SHA-1 hash of the given input.
 */
Uint8List sha1Digest(Uint8List input) => new SHA1Digest().process(input);
 ...
 */
Uint8List sha256hash160(Uint8List input) => ripemd160Digest(singleDigest(input));
