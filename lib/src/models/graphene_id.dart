...
class GrapheneId extends Serializable{
  ...
  factory GrapheneId.fromJson(dynamic json) {
    if(!(json is String)) {
      return null;
    }
    return GrapheneId(json);
  }
  ...
  bool serialize(Encoder encoder)  {
    encoder.encodeVarint(this.instance);
...
class GrapheneIds extends Serializable{
  List<GrapheneId> ids = List();
  ...
  factory GrapheneIds.fromJson(dynamic json) {
    if(!(json is List)) {
      return null;
    }

    GrapheneIds ids = GrapheneIds();
    for(var x in json) {
      final s = x as String;

      GrapheneId id = GrapheneId(s);
      ids.ids.add(id);
    }
    return ids;
  }
